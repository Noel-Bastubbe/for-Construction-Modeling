...
export default class DataObjectLabelHandler extends CommandInterceptor {
    constructor(eventBus, modeling, directEditing, overlays, goalModeler) {
        super(eventBus);
        ...
        this._nameDropdown = getDropdown("Names");
        this._dropdownContainer.appendChild(this._nameDropdown);
        this._currentDropdownTarget = undefined;
        this._overlayId = undefined;
        this._overlays = overlays;
        this._goalModeler = goalModeler;

        // eventBus.on('element.changed', function (e) {
        //     if (is(e.element, 'gm:Object') && e.element.parent) {
        ...
        eventBus.on('directEditing.activate', function (e) {
            if (is(e.active.element, 'gm:Object')) {
                ...
                const updateClassSelection = () => {
                    if (dataClasses.length > 0) {
                        ...
                        if (dataObject.dataclass) {
                            currentDataClass = dataClasses.filter(clazz => clazz.classRef === dataObject.dataclass)[0];
