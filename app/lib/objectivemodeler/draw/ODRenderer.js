...
export default function ODRenderer(
  ...
  function renderAttributes(parentGfx, element) {
    ...
    let label = "state: "
    if(semantic.states){
      label += semantic.states?.map(state => state.name).join(", ") || "no State"
    }
  ...
  this.handlers = {
    'om:Object': function(parentGfx, element, attrs) {
    ...
    'om:Link': function(parentGfx, element) {
    ...
    'om:TextBox': function(parentGfx, element) {
      var attrs = {
        fill: 'none',
        stroke: 'none'
      };
...
ODRenderer.prototype.canRender = function(element) {
  return is(element, 'om:BoardElement');
};
